...
All configuration lives in `src/config.js` and `.env` files:
...
**New FAQ:** Add an object to `faqItems` in `src/data/faqData.js` with `id`, `slug`, `category`, `question`, `answer`, and `seo`. It automatically gets a page, sitemap entry, and search indexing.

**New article:** Add to `src/data/articleData.js`. Same pattern.

**Quiz questions:** Edit via admin panel or directly in `src/data/quizData.js`.
