...
async function seed() {
  ...
  const { rows } = await sql`SELECT count(*) as count FROM quiz_questions`;
  ...
  for (let i = 0; i < quizQuestions.length; i++) {
    ...
    await sql`
      INSERT INTO quiz_questions (question, options, sort_order)
      VALUES (${q.question}, ${JSON.stringify(q.options)}::jsonb, ${i + 1})
    `;
  }
